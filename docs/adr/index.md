...
| [ADR-0046](0046-read-committed-isolation-targeted-row-locking-for-contended-paths.md) | `READ COMMITTED` isolation + targeted row-locking for contended paths | Accepted — COMPLETE: targeted row-locking (C1), the in-schema state guards (C2), and the worker/registrar role split with both column-immutability arms (C3, 2026-08-01) are all IMPLEMENTED |
